...
public class ExamAccountController {
     ...
     * @return
     */
    @RequestMapping(value = "/examToRepeat/{examid}/{creatorid}", method = RequestMethod.POST)
    public ResponseMsg examToRepeat(@PathVariable Integer examid, @PathVariable String creatorid) {
        logger.info("Enter into controller method examToRepeat.");
        ResponseMsg out = null;
        // TODO
        out = examAccountService.examToRepeat(examid, creatorid) ?  new ResponseMsg(CODE.RESP_SUC) : new ResponseMsg(CODE.UNEXP_ERR); 
        return out;
    }
    
    /**
     * 考试的人员添加-根据部门id查询员工，去除已经关联这次考试的人员
     * <功能详细描述>
     * @param examid
     * @return
     */
    @RequestMapping(value = "/examaccountlist/{examid}/{departmentid}", method = RequestMethod.POST)
    public ResponseMsg examAccountAddDep(@PathVariable Integer examid, @PathVariable Integer departmentid) {
        logger.info("Enter into controller method examAccountAddDep.");
        ResponseMsg out = new ResponseMsg(CODE.RESP_SUC);
        // TODO
        List<Account> dataList = examAccountService.examAccountAddDep(examid, departmentid);
     ...
     * @return
     */
    @RequestMapping(value = "/examaccountadd/{examid}/{creatorid}", method = RequestMethod.POST)
    public ResponseMsg examAccountAdd(@PathVariable Integer examid, @PathVariable String creatorid, @RequestBody List<String> accountIds) {
        logger.info("Enter into controller method examAccountAdd.");
        ResponseMsg out = null;
        // TODO
        out = examAccountService.examAccountAdd(examid, creatorid, accountIds) ?  new ResponseMsg(CODE.RESP_SUC) : new ResponseMsg(CODE.UNEXP_ERR); 
        return out;
    }
