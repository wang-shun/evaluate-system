...
public class SubjectDbserviceImpl implements SubjectDbService {
	...
	@Override
	public PageInfo<SubjectDb> queryAllSubjectDb(String subjectDbName,String creatorId, Integer isOpen, Integer pageNum, Integer pageSize) {

		try {
			pageNum = pageNum == null ? PageUtil.DEFULT_PAGENUM : pageNum;
			pageSize = pageSize == null ? PageUtil.DEFULT_PAGESIZE : pageSize;
			PageHelper.startPage(pageNum, pageSize);
			List<SubjectDb> list = subjectDbMapper.queryAllSubjectDb(subjectDbName, isOpen,creatorId);
			PageInfo<SubjectDb> info = new PageInfo<>(list);
			return info;
		} catch (Exception e) {
